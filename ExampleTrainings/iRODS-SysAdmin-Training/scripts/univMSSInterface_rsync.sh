...
export SCPCOMMAND=/usr/bin/rsync
...
syncToArch () {
	...
	#rsync -a -e "ssh -i $KEY" test.txt christin@archive.surfsara.nl:rsync_test.txt
	echo "UNIVMSS ${SCPCOMMAND} -a -e ssh -i ${KEY} ${1:?} ${USER}@${ARCHIVEADDRESS}:${2:?}"
	"${SCPCOMMAND}" -a -e "ssh -i ${KEY}" "${1:?}" "${USER}@${ARCHIVEADDRESS}:${2}"
	return
}
...
stageToCache () {
	...
	# e.g: /usr/local/bin/rfcp rfioServerFoo:${1:?} ${2:?}
        ...
        #`$op ${1:?} ${2:?}`
        #echo "UNIVMSS $op ${1:?} ${2:?}"
	echo "UNIVMSS ${SCPCOMMAND} -a -e ssh -i ${KEY} ${USER}@${ARCHIVEADDRESS}:${1:?} ${2:?}"
        "${SCPCOMMAND}" -a -e "ssh -i ${KEY}" "${USER}@${ARCHIVEADDRESS}:${1}" "${2:?}"
	return
}
